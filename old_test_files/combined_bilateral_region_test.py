"""
Combines both validated mechanisms:
1. Bilateral arc-comparison — on a genuine SAME-POINT collision,
   the side with the smaller own-arc wins (fixes rugby-style
   wraparound swaps).
2. Region-exclusion on advance — when a side loses and must
   advance to its next candidate, exclude the whole region around
   the point it lost, not just that exact coordinate (fixes
   hanging-shirt's multi-point cluster).

Run: python combined_bilateral_region_test.py
"""
...
COLLISION_RADIUS_PX = 15      # same-point collision detection
REGION_EXCLUSION_RADIUS_PX = 100  # applied only when advancing past a loss

AVATAR_USER_ID = "15bacab3-5873-401b-9c9a-52f8b3eeeaf7"

KNOWN_HEM_CORNERS = {
    "polo": [(378, 445), (93, 442)],
    "hanging_shirt": [(366, 458)],
}
...
def resolve_combined(left_candidates, right_candidates):
    """
    li/ri walk each side's list. On a genuine same-point collision,
    arc-distance decides the winner (bilateral). When a side loses
    — whether by collision or by region-exclusion — it advances
    past the WHOLE region around the point it lost, not just that
    exact coordinate.
    """
    li, ri = 0, 0
...
def main():
    ...
    for case in TEST_CASES:
        ...
        left_c = get_filtered_candidates(main_contour, left_idx, h_img, point_landmarks)
        right_c = get_filtered_candidates(main_contour, right_idx, h_img, point_landmarks)
