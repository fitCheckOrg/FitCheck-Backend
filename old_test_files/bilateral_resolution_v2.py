"""
Bilateral resolution v2 — directly motivated by the collision
diagnostic: on collision, keep the point for whichever side has
the SMALLER own-arc-distance to it; the losing side falls back to
its own next-best non-colliding candidate.

Run: python bilateral_resolution_v2.py
"""
...
def resolve_bilateral(left_candidates, right_candidates):
    """
    THE NEW RULE: pick each side's top candidate. If they collide
    (same physical point), keep it for whichever side has smaller
    own-arc; the losing side advances to its own next candidate
    and re-checks for collision, repeating until resolved.
    """
    li, ri = 0, 0
    while li < len(left_candidates) and ri < len(right_candidates):
        lc, ld, lpt = left_candidates[li]
        rc, rd, rpt = right_candidates[ri]
        collision = np.hypot(lpt[0]-rpt[0], lpt[1]-rpt[1]) < COLLISION_RADIUS_PX

        if not collision:
            return (lpt, lc, ld), (rpt, rc, rd)

        if ld <= rd:
            ri += 1  # RIGHT loses, advance RIGHT
        else:
            li += 1  # LEFT loses, advance LEFT
...
def main():
    ...
    for case in TEST_CASES:
        ...
        if "RIGHT" in out_of_scope:
            left_c = get_filtered_candidates(main_contour, left_idx, h_img, known_point_landmarks)
            c, d, pt = left_c[0]
        ...
        left_c = get_filtered_candidates(main_contour, left_idx, h_img, known_point_landmarks)
        right_c = get_filtered_candidates(main_contour, right_idx, h_img, known_point_landmarks)

        left_result, right_result = resolve_bilateral(left_c, right_c)

        for side_name, result, confirmed in [
            ("LEFT", left_result, case["confirmed_cuff"]["LEFT"]),
            ("RIGHT", right_result, case["confirmed_cuff"]["RIGHT"])
        ]:
            total += 1
            if result is None:
