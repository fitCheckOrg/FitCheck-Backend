"""
v7 — X-return boundary, replacing both v5's threshold-crossing and
v6's fixed-window approaches.

Search window ends when the contour crosses back past the
underarm's own x-coordinate — a real structural signature (the
sleeve going out and returning), not a tuned distance constant.
Verified by hand against all three existing profiles before
writing this code.

Run: python sleeve_arc_isolation_v7.py
"""
...
def find_x_return_boundary(contour, sleeve_arc, underarm_x, going_positive):
    """
    THE NEW CRITERION: find where x first crosses back past
    underarm_x, AFTER having moved away from it. going_positive
    indicates whether the sleeve initially moves in +x or -x
    direction (determined from early samples).
    """
    ...
    return ordered[-1][1]  # never crossed back — use full arc
...
def main():
    for case in TEST_CASES:
        ...
        for side_name, underarm_idx, color, going_positive in [
            ("LEFT", left_idx, (255, 0, 0), False),
            ("RIGHT", right_idx, (0, 200, 255), True)
        ]:
            ...
            direction_name = "FORWARD" if fwd_disp > bwd_disp else "BACKWARD"
            ...
            print(f"{side_name} ({direction_name}): x-return boundary at {window_end:.1f}px  "
                  f"peak at arc={stop_d:.1f}px pos={stop_point} curvature={stop_c:.1f}°")
        ...
        filename = f"sleeve_arc_v7_{case['label']}.png"
