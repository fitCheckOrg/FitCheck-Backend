"""
Surveys all 8 confirmed cuffs: for each, is there a comparably-
strong second peak nearby (candidate pair, like hanging-shirt 
RIGHT), or is the confirmed cuff isolated (single-point sufficient)?

Pure classification of existing ground truth. No production 
changes, no new algorithm.

Run: python cuff_pair_survey.py
"""
...
TOP_K = 8
PAIR_PIXEL_RADIUS = 120     # "nearby" in physical space
PAIR_CURVATURE_RATIO = 0.85  # "comparably strong" = within this fraction
...
def main():
    print(f"{'garment/side':<22}{'confirmed cuff':<18}{'curvature':<12}"
          f"{'2nd-strongest nearby':<26}{'gap':<10}{'classification'}")
    print("-" * 100)
        ...
        for side_name, underarm_idx in [("LEFT", left_idx), ("RIGHT", right_idx)]:
            ...
            for c, d, idx in peaks:
                ...
                if np.hypot(pt[0]-confirmed[0], pt[1]-confirmed[1]) < 15:
                    confirmed_curv = c
                    break
            if confirmed_curv is None:
                continue

            # Find nearby comparably-strong peaks
            best_second, best_second_dist = None, None
            for c, d, idx in peaks:
                pt = tuple(int(v) for v in main_contour[idx][0])
                pixel_dist = np.hypot(pt[0]-confirmed[0], pt[1]-confirmed[1])
                if 0 < pixel_dist < PAIR_PIXEL_RADIUS and c >= confirmed_curv * PAIR_CURVATURE_RATIO:
                    if best_second is None or c > best_second[0]:
                        best_second = (c, pt)
                        best_second_dist = pixel_dist

            label = f"{case['label']} {side_name}"
            if best_second:
                classification = "PAIR (two real corners)"
                print(f"{label:<22}{str(confirmed):<18}{confirmed_curv:<12.1f}"
                      f"{str(best_second[1])+f' ({best_second[0]:.1f}°)':<26}"
                      f"{best_second_dist:<10.1f}{classification}")
            else:
                classification = "SINGLE (isolated)"
                print(f"{label:<22}{str(confirmed):<18}{confirmed_curv:<12.1f}"
                      f"{'none':<26}{'—':<10}{classification}")
