"""
Cuff detection v2 — adds a vertical constraint to the max-distance
hypothesis, directly targeting the confirmed failure mode: the hem
sharing the sleeve's column range and being selected instead of the
real cuff on short sleeves.

New constraint: candidates must be ABOVE the garment's own hem 
y-coordinate (already detected via existing, trusted hem-detection 
code). The hem itself can now never be selected as a cuff, 
regardless of distance.

Run: python cuff_detection_test_v2.py
"""
...
def find_hem(main_contour):
    """Existing, trusted hem detection — lowest contour point, 
    used tonight without issue on multiple garments."""
    lowest_idx = np.argmax(main_contour[:, 0, 1])
    return tuple(int(v) for v in main_contour[lowest_idx][0])


def find_cuff_in_column_range(main_contour, underarm_point, x_min, x_max, hem_y, margin=15):
    """
    v2: same max-distance logic, PLUS candidates must sit strictly
    above (hem_y - margin) — a small margin so points right at the
    hem edge, which naturally exist in the same column, don't get
    selected even if the hem's exact contour index isn't the exact
    ...
    for pt in main_contour[:, 0, :]:
        ...
        if x_min <= x <= x_max and y < (hem_y - margin):
...
def main():
    for case in TEST_CASES:
        ...
        hem = find_hem(main_contour)
        print(f"left_underarm: {left_ua}  right_underarm: {right_ua}  hem: {hem}")

        left_sleeve_range = (0, int(w_img * 0.20))
        right_sleeve_range = (int(w_img * 0.80), w_img)

        left_cuff, left_dist = find_cuff_in_column_range(main_contour, left_ua, *left_sleeve_range, hem[1])
        right_cuff, right_dist = find_cuff_in_column_range(main_contour, right_ua, *right_sleeve_range, hem[1])

        print(f"\nleft_cuff (v2): {left_cuff}  dist={left_dist:.1f}px  "
              f"pos=({left_cuff[0]/w_img*100:.1f}%, {left_cuff[1]/h_img*100:.1f}%)")
        print(f"right_cuff (v2): {right_cuff}  dist={right_dist:.1f}px  "
        ...
        filename = f"cuff_test_v2_{case['label']}.png"
