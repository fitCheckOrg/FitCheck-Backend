"""
Tests whether the EXISTING, unmodified curvature-peak logic finds
the cuff once given the CORRECT rugby sleeve arc (LEFT=FORWARD,
RIGHT=BACKWARD, confirmed via full trajectory trace).

CURVATURE_WINDOW, MIN_PEAK_SEPARATION unchanged from every prior
test tonight. No new heuristic. Pure measurement of the existing
mechanism on corrected input.

Run: python rugby_correct_arc_curvature_test.py
"""
...
# Forced correct directions — confirmed via full trajectory trace,
# NOT via classify_direction()
CONFIRMED_SLEEVE_DIRECTION = {"LEFT": "FORWARD", "RIGHT": "BACKWARD"}
...
def main():
    ...
    for side_name, underarm_idx, going_positive in [
        ("LEFT", left_idx, False),   # corrected: x drops first, returns by rising
        ("RIGHT", right_idx, True)   # corrected: x rises first, returns by dropping
    ]:
        ...
        forced_direction = CONFIRMED_SLEEVE_DIRECTION[side_name]
        sleeve_arc = forward if forced_direction == "FORWARD" else backward

        window_end = find_x_return_boundary(main_contour, sleeve_arc, underarm_x, going_positive)

        print(f"\n{'='*70}")
        print(f"{side_name} — underarm={underarm_pt}  FORCED direction={forced_direction}  "
              f"boundary={window_end:.1f}px")
        print(f"{'='*70}")
