"""
Tests: among top-K unbounded curvature peaks (properly filtered 
for MIN_ARC_BEFORE_PEAK_SEARCH), select the one CLOSEST in arc-
distance to the underarm — not the one with highest curvature.

Reuses only already-validated pieces: Layer 1 direction, unbounded 
curvature computation, proper peak-separation logic. No new 
geometric feature — just a different selection rule among peaks 
already known to reliably contain the true cuff.

Run: python arc_proximity_selection_test.py
"""
...
def main():
    ...
    for case in TEST_CASES:
        ...
        for side_name, underarm_idx in [("LEFT", left_idx), ("RIGHT", right_idx)]:
            ...
            # THE NEW RULE: smallest arc-distance among top-K, not highest curvature
            peaks_by_arc = sorted(peaks, key=lambda p: p[1])
            chosen = peaks_by_arc[0] if peaks_by_arc else None
            ...
            print(f"\n  {side_name} — top-{TOP_K} peaks (curvature-ranked):")
            for c, d, idx in peaks:
                pt = tuple(int(v) for v in main_contour[idx][0])
                marker = " <-- CHOSEN (smallest arc)" if (c, d, idx) == chosen else ""
                print(f"    arc={d:6.1f}  pos={pt}  curvature={c:.1f}°{marker}")
            print(f"  confirmed={confirmed}  found={found_pt}  dist={dist:.1f}px  "
                  f"{'✅' if match else '❌'}")
