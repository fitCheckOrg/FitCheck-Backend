"""
Fixed version: adds the collar/shoulder-zone exclusion (y_pct < 15%)
that the real bilateral pipeline already uses, which v1 of this
diagnostic omitted — causing rugby's whole collar cluster to appear
as false "unexplained" noise.

Run: python q3_landmark_crossref_v2.py
"""
...
COLLAR_ZONE_Y_PCT = 15  # matches the real pipeline's exclusion

AVATAR_USER_ID = "15bacab3-5873-401b-9c9a-52f8b3eeeaf7"
KNOWN_HEM_CORNERS = {"polo": [(378, 445), (93, 442)]}
CONFIRMED_CUFFS = {
    "polo": {"LEFT": (31, 154), "RIGHT": (439, 142)},
    "hanging_shirt": {"LEFT": (26, 457), "RIGHT": (442, 447)},
    "rugby": {"LEFT": (51, 488), "RIGHT": (430, 459)},
    "dress_shirt": {"LEFT": (54, 633), "RIGHT": (166, 660)},
}
...
def main():
    unexplained_pairs = []
    explained_count = 0
    collar_excluded_count = 0

    for case in TEST_CASES:
        label = case["label"]
        print(f"\n{'='*75}")
        print(f"{case['item_id']} — {label}")
        print('='*75)
        ...
        known_landmarks = underarms + KNOWN_HEM_CORNERS.get(label, [])
        confirmed = CONFIRMED_CUFFS[label]
        ...
        for side_name, underarm_idx in [("LEFT", left_idx), ("RIGHT", right_idx)]:
            ...
            for i in range(len(peaks_by_arc) - 1):
                ...
                pt1_is_cuff = np.hypot(pt1[0]-side_confirmed[0], pt1[1]-side_confirmed[1]) < 15
                pt2_is_cuff = np.hypot(pt2[0]-side_confirmed[0], pt2[1]-side_confirmed[1]) < 15
                touches_cuff = pt1_is_cuff or pt2_is_cuff

                if pt1_matches or pt2_matches:
                    explained_count += 1
                    print(f"  {side_name}: gap={gap:.1f}px  {pt1}<->{pt2}  — EXPLAINED (known landmark)")
                else:
                    tag = "  [TOUCHES CONFIRMED CUFF]" if touches_cuff else ""
                    unexplained_pairs.append((label, side_name, gap, pt1, pt2, c1, c2, touches_cuff))
                    print(f"  {side_name}: gap={gap:.1f}px  {pt1}(c={c1:.1f}°)<->{pt2}(c={c2:.1f}°)"
                          f"  — UNEXPLAINED{tag}")

    print(f"\n\n{'='*75}")
    print(f"SUMMARY: {explained_count} landmark-explained, "
          f"{collar_excluded_count} collar-zone excluded, "
          f"{len(unexplained_pairs)} genuinely unexplained")
    print('='*75)

    cuff_touching = [p for p in unexplained_pairs if p[7]]
    non_cuff = [p for p in unexplained_pairs if not p[7]]

    print(f"\n--- Pairs touching a confirmed cuff ({len(cuff_touching)}) ---")
    for label, side, gap, pt1, pt2, c1, c2, _ in sorted(cuff_touching, key=lambda x: x[2]):
        print(f"  {label} {side}: gap={gap:.1f}px  {pt1}(c={c1:.1f}°) <-> {pt2}(c={c2:.1f}°)")

    print(f"\n--- Pairs NOT touching any confirmed cuff ({len(non_cuff)}) — "
          f"these are what a merge rule must not damage ---")
    for label, side, gap, pt1, pt2, c1, c2, _ in sorted(non_cuff, key=lambda x: x[2]):
        print(f"  {label} {side}: gap={gap:.1f}px  {pt1}(c={c1:.1f}°) <-> {pt2}(c={c2:.1f}°)")
