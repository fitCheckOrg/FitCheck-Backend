"""
Diagnostic (not yet a rule): for each side, lists ALL candidates
sorted by ARC-DISTANCE first (ascending), showing curvature next
to each. Checks whether the smallest-arc candidate is already a
genuine peak (curvature well above noise level) — testing whether
arc-distance alone, applied per-side to that side's OWN candidate
list, resolves the bilateral swap problem without any weighted
combination or guessed coefficient.

Run: python arc_first_diagnostic.py
"""
...
def main():
    ...
    for case in TEST_CASES:
        ...
        for side_name, underarm_idx in [("LEFT", left_idx), ("RIGHT", right_idx)]:
            ...
            # Sort by ARC-DISTANCE first, not curvature
            peaks_by_arc = sorted(peaks, key=lambda p: p[1])

            confirmed = case["confirmed_cuff"][side_name]
            print(f"\n  {side_name} — candidates sorted by ARC-DISTANCE (ascending):")
            for c, d, idx in peaks_by_arc:
                pt = tuple(int(v) for v in main_contour[idx][0])
                is_confirmed = np.hypot(pt[0]-confirmed[0], pt[1]-confirmed[1]) < 15
                marker = "  <-- CONFIRMED CUFF" if is_confirmed else ""
                print(f"    arc={d:7.1f}  curvature={c:6.1f}°  pos={pt}{marker}")

            smallest_arc_candidate = peaks_by_arc[0]
            found_pt = tuple(int(v) for v in main_contour[smallest_arc_candidate[2]][0])
            ...
            if match:
                correct += 1
            print(f"  → Smallest-arc candidate: {found_pt}  confirmed={confirmed}  "
                  f"dist={dist:.1f}px  {'✅' if match else '❌'}")

    print(f"\n\n{'='*75}")
    print(f"FINAL (smallest-arc-per-side, no weighting): {correct}/{total}")
    print('='*75)
