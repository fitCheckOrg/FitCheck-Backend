"""
Cuff detection v3 — replaces the single global hem exclusion with
a LOCAL hem height, computed separately within each sleeve's own
column range. Directly targets the confirmed v2 asymmetry: a single
global hem point + small margin wasn't sufficient when the true
lowest point wasn't centered, leaving other near-hem points on one
side free to win.

Run: python cuff_detection_test_v3.py
"""
...
def find_local_hem_y(main_contour, x_min, x_max):
    """NEW: lowest y-coordinate WITHIN this specific column range,
    not the garment's single global lowest point. This is the real
    local floor for whichever side is being checked."""
    ys_in_range = [int(pt[0][1]) for pt in main_contour if x_min <= pt[0][0] <= x_max]
    return max(ys_in_range) if ys_in_range else None


def find_cuff_in_column_range(main_contour, underarm_point, x_min, x_max, local_hem_y, margin=15):
    ...
    for pt in main_contour[:, 0, :]:
        ...
        if x_min <= x <= x_max and y < (local_hem_y - margin):
            dist = np.hypot(x - ux, y - uy)
            print(f"  candidate: ({x},{y}) dist={dist:.1f}")  # TEMPORARY — see every candidate
            if dist > best_dist:
                best_dist = dist
                best_point = (x, y)
...
def main():
    for case in TEST_CASES:
        ...
        left_local_hem = find_local_hem_y(main_contour, *left_sleeve_range)
        right_local_hem = find_local_hem_y(main_contour, *right_sleeve_range)
        print(f"left_local_hem_y: {left_local_hem}  right_local_hem_y: {right_local_hem}")

        left_cuff, left_dist = find_cuff_in_column_range(main_contour, left_ua, *left_sleeve_range, left_local_hem)
        right_cuff, right_dist = find_cuff_in_column_range(main_contour, right_ua, *right_sleeve_range, right_local_hem)

        print(f"\nleft_cuff (v3): {left_cuff}  dist={left_dist:.1f}px  "
              f"pos=({left_cuff[0]/w_img*100:.1f}%, {left_cuff[1]/h_img*100:.1f}%)")
        print(f"right_cuff (v3): {right_cuff}  dist={right_dist:.1f}px  "
        ...
        filename = f"cuff_test_v3_{case['label']}.png"
