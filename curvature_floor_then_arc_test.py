"""
Two-stage test: (1) filter candidates below a per-side curvature
floor (fraction of that side's own max curvature — self-
calibrating, not a fixed degree threshold), (2) among survivors,
pick smallest arc-distance.

Floor fraction = 0.5 for this first test — explicitly an
experimental value, not asserted as validated.

Hanging-shirt's adjacent-peak ambiguity (two real, strong, nearby
peaks) is explicitly NOT expected to be fixed by this change and
is reported separately, not folded into pass/fail silently.

Run: python curvature_floor_then_arc_test.py
"""
...
TOP_K = 8
CURVATURE_FLOOR_FRACTION = 0.5  # EXPERIMENTAL — fraction of this 
                                   # side's own max curvature

AVATAR_USER_ID = "15bacab3-5873-401b-9c9a-52f8b3eeeaf7"

KNOWN_ADJACENT_PEAK_CASES = {("hanging_shirt", "LEFT"), ("hanging_shirt", "RIGHT")}
...
def main():
    ...
    for case in TEST_CASES:
        ...
        for side_name, underarm_idx in [("LEFT", left_idx), ("RIGHT", right_idx)]:
            ...
            max_curvature = max(c for c, d, idx in peaks)
            floor = max_curvature * CURVATURE_FLOOR_FRACTION
            survivors = [(c, d, idx) for c, d, idx in peaks if c >= floor]
            survivors_by_arc = sorted(survivors, key=lambda p: p[1])

            confirmed = case["confirmed_cuff"][side_name]
            chosen = survivors_by_arc[0]
            found_pt = tuple(int(v) for v in main_contour[chosen[2]][0])
            dist = np.hypot(found_pt[0]-confirmed[0], found_pt[1]-confirmed[1])
            match = dist < 15
            ...
            is_known_ambiguous = (case["label"], side_name) in KNOWN_ADJACENT_PEAK_CASES
            flag = "  [KNOWN ADJACENT-PEAK CASE — not expected to be fixed by this test]" \
                   if is_known_ambiguous else ""
            if is_known_ambiguous and not match:
                known_ambiguous_flagged += 1

            print(f"\n  {side_name}: max_curvature={max_curvature:.1f}°  floor={floor:.1f}°  "
                  f"{len(peaks)} peaks -> {len(survivors)} survive floor{flag}")
            print(f"    chosen={found_pt}  confirmed={confirmed}  dist={dist:.1f}px  "
                  f"{'✅' if match else '❌'}")

    print(f"\n\n{'='*75}")
    print(f"FINAL: {correct}/{total}  (of which {known_ambiguous_flagged} "
          f"failure(s) are pre-flagged known adjacent-peak cases, not newly introduced)")
    print('='*75)
