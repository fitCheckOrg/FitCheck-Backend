"""
Visual diagnosis for the collar_shoulder triple-cluster question.

2a32b5bb's top-5 showed THREE separate points (B#6, B#7, B#8) all
classified as collar_shoulder, all within y=1.2-8.3%. Are these
three genuinely distinct real landmarks, or the same feature
detected multiple times at slightly different points?

Marks all three on the actual garment image for direct visual check.

Run: python diagnose_collar_cluster.py
"""
...
ITEM_ID = "2a32b5bb-c3ea-48f9-ac8d-5849eee866ba"
...
def main():
    ...
    for i, (score, event, reasons) in enumerate(scored[:5]):
        x, y = event["px"]
        color = colors[i % len(colors)]
        draw.ellipse([x-10, y-10, x+10, y+10], outline=color, width=3)
        draw.text((x+12, y-8), f"{event['path_id']}#{event['rank']} ({score})", fill=color)

    canvas.save("collar_cluster_diagnosis.png")
    print("Saved: collar_cluster_diagnosis.png")
