"""
Tests the landmark-exclusion hypothesis: remove candidates that
coincide with already-known landmarks of a DIFFERENT type (own/
opposite underarm, own hem corner if independently known, opposite
side's own cuff via wraparound) BEFORE ranking by curvature.

This is the first Layer 2 hypothesis built from identified feature
IDENTITY rather than a distance/closeness threshold.

Dress-shirt RIGHT remains explicitly out of scope (signal-absence,
not a selection problem).

Run: python landmark_exclusion_test.py
"""
...
TOP_K = 8
EXCLUSION_RADIUS_PX = 15  # candidate within this of a known landmark = excluded

AVATAR_USER_ID = "15bacab3-5873-401b-9c9a-52f8b3eeeaf7"

# Known hem corners (independently confirmed, only available for polo
# from earlier visual-verification work)
KNOWN_HEM_CORNERS = {
    "polo": [(378, 445), (93, 442)],  # A1, A2 in pixel space (from 
                                         # earlier % conversion)
}
...
def main():
    ...
    for case in TEST_CASES:
        ...
        known_landmarks = list(underarms) + KNOWN_HEM_CORNERS.get(case["label"], [])

        for side_name, underarm_idx in [("LEFT", left_idx), ("RIGHT", right_idx)]:
            if side_name in out_of_scope:
                print(f"\n  {side_name} — OUT OF SCOPE (signal-absence case)")
                continue
            ...
            # THE EXCLUSION STEP — remove candidates matching known landmarks
            filtered = []
            ...
            print(f"\n  {side_name}: {len(peaks)} raw candidates -> "
                  f"{len(filtered)} after excluding known landmarks")
